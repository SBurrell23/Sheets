# Arranging spec — Hymns

This collection is **not** original composition. Each entry is a hymn tune or gospel song
that is out of copyright, arranged as a beginner–intermediate lead sheet: a single-line
melody in the treble clef with chord symbols above it. No left-hand part.

Your job is to transcribe a tune faithfully, not to invent one.

**Hymns are published in four-part harmony. Take the SOPRANO line and nothing else.** That
is the part a congregation sings and the only one anybody hums. The alto, tenor and bass
...
A hymn is strophic: the same tune carries every verse. **Write the tune once.** Do not write
out verse after verse — they are the same notes.

Where a gospel song has a **refrain** that differs from the verse (Blessed Assurance, The Old
Rugged Cross, What a Friend does not, Great Is Thy Faithfulness does), write the verse once and
the refrain once. That is the whole song. Expect 8 to 32 bars; a plain hymn tune like Old
Hundredth is 8 and that is correct, not short.
...
Write exactly one file to the path you are given, `collections/hymns/songs/<slug>.json`:
...
- `source` — the composer and date of the TUNE, its tune name in capitals if it has one
  (NICAEA, BETHANY, CWM RHONDDA — hymnals index by tune, and two hymns often share one), and
  the author of the words where they are well known.
...
```bash
python src/validate.py collections/hymns/songs/<slug>.json
```
...
2. **hymnary.org** indexes essentially every hymn tune by name and carries page scans plus,
   for many, a MIDI or a score. **CyberHymnal / hymntime.com** carries MIDI and text for
   thousands. Search by the TUNE name as well as the first line.
