...
```
index.html                     the page shell -- markup and script tags, ~4KB
favicon.svg
assets/
  app.css
  js/data.js                   the data registry and lazy loader
  js/art.js                    per-collection artwork
  js/player.js                 the player
data/                          generated
  collections.js               the manifest: titles, sets, song metadata
  <collection>.js              that collection's ABC and spec text
collections/                   source of truth, one folder per collection
  folk-songs/
...
src/
  ...
  build.py                     validates everything, engraves PDFs, writes data/
...
working when you double-click it. A classic `<script src>` is **not** blocked, so the song data is
emitted as plain `.js` files that assign to globals.

That buys lazy loading for free. `data/collections.js` is the manifest and is always loaded;
`data/<collection>.js` holds that collection's notation and is pulled in by injecting a script tag
the first time you open it. Opening the page loads the manifest and one collection, not all of
them — which matters more with every collection added.

`index.html` and `assets/` are hand-written source. `data/` and `songs/` are generated by
`build.py`; do not edit them.
